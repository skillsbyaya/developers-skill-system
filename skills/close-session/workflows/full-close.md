...
An active multi-packet story waiting for another packet or its story-completion session remains uncommitted only where the project's declared Git workflow makes the whole story the landing unit; where it lands each packet, the session ends with its work landed. Do not read Git close or create a packet commit merely because the session is ending. Use a local checkpoint commit only when Agent Dev recorded a concrete recovery, concurrent-work, branch-switching, or large-overlapping-diff need.
...
- Always end with `### Handoff`, using a one-line blockquote with `**New session:**`, `**You:**`, or `**Waiting for <actor>:`. Write a `**New session:**` line as `<subject in plain words> — <one pasteable sentence naming the verb, the unit, the delivery skill and the record>`. Naming the skill carries its procedure: never restate the gate, record update, close, stop or completion method, and never repeat state already written where the next session will read it. When no explicit next item exists or ordering is ambiguous, that choice belongs in the round of questions asked above; write the answer as the `**New session:**` line. Use `> **You:** Decide where the project should go next.` only when the user declined to choose.
