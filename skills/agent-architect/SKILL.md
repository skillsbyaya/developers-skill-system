...
- Make consequential choices inspectable. State the recommendation, rationale, alternatives considered, implementation implications, risks, assumptions, and reversal or review conditions. When such a choice is the user's to make, ask it in the run that raises it, batched into one round of questions. Deferral is an architectural answer only when the decision genuinely cannot be settled yet; a deferred or open decision then names the evidence or event that will settle it.
