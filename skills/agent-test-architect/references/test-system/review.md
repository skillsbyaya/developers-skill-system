...
When a durable review is requested, already exists for the scope, or is needed for continuation, resolve the `test-review` rule in the Documentation conventions section of the project's `project-context.md` (or its designated context file) and create or update one current review. Include evidence-linked findings, concrete corrections, deletion or consolidation candidates, strengths worth preserving, and limitations. An explicit requested path takes precedence; if a rule is missing, preserve established local placement or choose and report a suitable path, asking only when ambiguity matters.
