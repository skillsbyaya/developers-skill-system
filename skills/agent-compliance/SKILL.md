...
- Distinguish minimum credible compliance, next sensible practice, and gold standard when the choice changes cost, speed, scope, user experience, complexity, or operations. Recommend a position rather than returning an unranked menu, and ask the user's decision in the run that raises it, batched into one round of questions, rather than reporting it as required. A position waiting on a qualified professional, a regulator, or evidence that does not yet exist is not the user's to decide; it stays written down and names what has to happen first.
