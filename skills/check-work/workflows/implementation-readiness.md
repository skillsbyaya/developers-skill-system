...
Identify the initiative, feature, epic set, or other bounded planning scope and the current sources that govern it. Start with the explicit planning set, its indexes, and the sections or summaries that identify requirements, decisions, stories, dependencies, and evidence expectations. Expand only into artifacts whose claims could change the verdict. Read a whole artifact when its structure, cross-references, or competing versions make partial inspection unreliable; do not make complete reading of every selected source a default gate. Do not require an artifact merely because it is conventional; record it as absent only when the scoped work needs the decision it should contain.

When the scope is expressed as one canonical project area rather than an explicit source set, resolve [the identifier convention](../../organise-docs/references/identifier-conventions.md) and the project `identifier-areas` registry. Use the exact dotted area segment to route candidate current IDs across requirements, decisions, architecture, delivery, reviews, and risks; triage out archived, superseded, and false-positive matches, then inspect the authoritative portions needed for the readiness decision. Follow cross-area dependencies only when they can affect the scoped implementation, and treat missing suffixes as discoverability gaps rather than proof that a source is irrelevant.
