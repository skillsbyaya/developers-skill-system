...
Use a PR, commit, branch, diff, or implementation artifact only as supporting evidence for what should be observable and which build contains it. Never ask the person to examine source control, code, logs, or automated checks. If no relevant environment is available, name what is missing and do not substitute artifact review for human preview.
...
6. Keep explanations, optional things the person may notice, and specialist or unavailable-environment evidence outside the required checklist. A delegated check must remain an observable, human-suitable action.
