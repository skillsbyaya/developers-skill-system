...
- When writing a durable artifact, use the Documentation conventions section of the project's `project-context.md` (or its designated context file), reuse an existing report for the same scope, and use a bundled template only when it improves the result. An explicit requested path takes precedence; if a rule is missing, preserve established local placement or choose and report a suitable path, asking only when ambiguity matters.
