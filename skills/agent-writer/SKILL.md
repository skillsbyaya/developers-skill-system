...
Keep neighbouring work independent: `organise-docs` owns documentation-set restructuring, archival, deletion, and broad maintenance; ordinary document locations and naming come directly from project context; `research` and `agent-dev investigation` acquire missing evidence; and creative or literary writing remains outside this skill. If a required owner is unavailable, state the boundary and provide only responsible bounded help.
