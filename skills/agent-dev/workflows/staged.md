...
If considering a clean-context implementation or verification worker, read [Worker execution](../references/worker-execution.md). Do not load it for inline delivery. After the last implementation slice, the packet-close handoff starts a separate story-completion session to run required integrated checks before applying the completion rule and delivery handoff in the delivery controls. When context ends or a block remains, leave the story or package at one precise safe boundary rather than creating a separate handoff log.
