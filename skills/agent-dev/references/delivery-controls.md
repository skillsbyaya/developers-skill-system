...
Stop at the first sufficient solution: no change; remove or replace obsolete code; reuse a project solution — look before you write, because re-implementing a component, pattern, or helper that already lives a few files over is the most common slop, and when new code does overlap one, say why you built rather than reused; use native or standard-library behaviour; use an installed dependency; make the smallest local change; add an abstraction, dependency, configuration surface, or scaffold only when the approved outcome or risk requires it. Fix defects at the lowest shared cause that preserves intended behaviour.
...
**Keep the record current with the code; commit them together.** Update the story or package, delivery-status entry, and known stale orientation pointers during each packet. For a multi-packet owner, follow [Packet coordination](packet-coordination.md): its default defers the commit until owner completion while code and records remain current and uncommitted. Describe implemented behaviour, supported lifecycle state, and next work; do not claim a commit, merge, or deployment that has not happened, or duplicate Git's merge metadata in the record.

Before any packet handoff or landing, inspect its delta for completion-only mutations: `done`, an archive move, active-status removal, backlog close-out, or an orientation pointer beyond the owner. Remove them unless this is the separate owner-completion boundary, and refresh affected document checks. Broad tests do not widen a packet's lifecycle authority.

After an actual landing, verify any required external outcome at the existing authority boundary. A matching observation needs no record rewrite. If the observed state differs, record it and make correction of that landed unit the exact next boundary; complete an authorised correction when safe, otherwise preserve its blocker. This exceptional correction does not require committing an otherwise unfinished story merely to make the worktree clean.
