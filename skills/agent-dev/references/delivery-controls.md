...
**Write the record so the landing unit can hold it.** A document that has to be corrected after the merge was written about the wrong subject. Describe what the change is, what it does, the state it leaves the owner in and the next boundary — all of that is true the moment the work is done, so all of it goes in the commit. Do not describe where the change went or that it arrived: a commit SHA, a pull-request number and a passing gate run are the repository's own record of the merge, and a merged pull request under a gated workflow already asserts the last of them. Writing them into a document creates text that cannot go in the commit it describes, and duplicates what git answers exactly. This does not report unfinished work as done — the record never claims the merge; the commit carrying it is what makes the record true. If gates fail, amend the branch before it lands.

After landing, verify what the record anticipated: a migration head, a deployment, an external checkpoint. A matching observation needs no writing. A diverging one is news rather than bookkeeping, so land that correction as the next boundary while the evidence is current. Never hold a corrected document across a session boundary as uncommitted state — a dirty tree is a claim on the next session's branch, and non-linear work, another story, a branch switch, a worktree or a parallel session, then has to carry it or work around it.
