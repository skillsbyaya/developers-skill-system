...
Packets are context, evidence, and continuation boundaries; whether one is also a landing boundary follows the project's declared Git workflow and the user's delivery authority. Where that workflow lands each packet, land it — code, record update, status transition and any stale orientation pointer in one commit — and end the session with a clean tree. Where the workflow makes the whole story the landing unit, packets accumulate uncommitted, and that carries a real cost against the review value of one whole-story diff: everything held is a claim on the next session's branch, so a branch switch, a worktree, a parallel session or a turn to another story has to carry it or work around it. Use a local packet checkpoint commit when recovery, concurrent work, branch switching, or a large overlapping cumulative diff creates a concrete need; do not push or open a packet-only pull request merely to mark a boundary.
