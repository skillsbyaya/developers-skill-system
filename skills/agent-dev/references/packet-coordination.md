...
At the packet boundary, invoke `close-session` packet close without waiting for the user to prompt it. That workflow preserves only continuation-critical learning and returns the paste-ready prompt for the next packet or the story-completion boundary above. Stop after the packet close. Use a bounded worker only under the worker reference; a pause in the same conversation does not reset context.
